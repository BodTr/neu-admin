                    ...
                    <div class="mb-3">
                      ...
                      <div class="card">
                        ...
                        <input
                          type="text"
                          class="form-control"
                          v-model="importDocumentsDocName"
                          disabled
                        />
                      </div>
...
export default {
  ...
  data() {
    return {
      ...
      content: "",
      expireIn: "",
      importDocumentsDoc: null,
      importDocumentsDocName: "",
      importDocMessage: "",
      displayModal: false,
  ...
  methods: {
    ...
    handleExcelUpload() {
      this.$refs.importDocumentsDoc.click();
    },
    handlePdfChange() {
    ...
    handleExcelChange() {
      const file = this.$refs.importDocumentsDoc.files[0]
      console.log(file, "file handleExcelChange()")
      const allowedTypes = ["application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"]
      const MAX_SIZE = 20 * 1024 * 1024;
      const tooLarge = file.size > MAX_SIZE;
      this.importDocumentsDoc = file;
      this.importDocumentsDocName = file.name
      if (allowedTypes.includes(file.type) && !tooLarge) {
    ...
    async getExcelFile() {
      try {
        ...
        const result = await instance.get("/api/export-excel-documents", {
          params: queryParams,
        });
    ...
    async downloadTemplate() {
      try {
        const result = await instance.get("/api/get-documents-template")
        ...
        console.log(
          error,
          "/api/get-documents-template catch block error"
        );
      }
    },
    async importFile() {
      try {
        let formData = new FormData();
        formData.append("document-import-file", this.importDocumentsDoc)
        formData.append("programId", this.id)
        const result = await instance.post("/api/import-documents-data", formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          }
        })
